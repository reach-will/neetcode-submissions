class Solution:
    def backtrack(self) -> None:
        ...
        for col in range(self.n):
            if not self.is_valid_path(len(self.path), col):
                continue

            self.path.append(col)

            self.cols.add(col)
            self.diags.add(len(self.path) - 1 - col)
            self.anti_diags.add(len(self.path) - 1 + col)

            self.backtrack()

            self.anti_diags.remove(len(self.path) - 1 + col)
            self.diags.remove(len(self.path) - 1 - col)
