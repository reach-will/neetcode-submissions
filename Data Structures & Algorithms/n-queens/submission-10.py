class Solution:
    def backtrack(self, row) -> None:
        if len(self.path) == self.n:
            self.result.append(["." * i + "Q" + "." * (self.n - i - 1) for i in self.path])
            return

        for col in range(self.n):
            if not self.is_valid_path(row, col):
                continue

            self.path.append(col)

            self.cols.add(col)
            self.diags.add(row - col)
            self.anti_diags.add(row + col)

            self.backtrack(row + 1)

            self.anti_diags.remove(row + col)
            self.diags.remove(row - col)
